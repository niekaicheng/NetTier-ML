...
\textbf{Method}: 5$\times$3 Nested Cross-Validation (outer 5-Fold evaluation, inner 3-Fold tuning), search space of 1,296 combinations, including \texttt{n\_estimators}, \texttt{max\_depth}, \texttt{class\_weight} and other parameters. E16 additionally employs Time-aware Split for full-data training. The results are summarized in Table~\ref{tab:s1_baseline}.
...
\textbf{Method}: Architecture Generalization --- same architecture, zero modifications, trained from scratch on UNSW-NB15 (149K/26K/82K, 186 features, 10 classes). Performance across datasets is reported in Table~\ref{tab:cross_gen}.
...
\textbf{Method}: Integrated Gradients (captum, 405 samples $\times$ 50 steps) + Attention Rollout \cite{25} + ECA Channel Attention \cite{23}. A comparison of attribution methods is provided in Table~\ref{tab:interpretability_comparison}.
...
\textbf{Method}: FGSM \cite{24} (single-step) + PGD \cite{13} (5 steps) on TransECA-Net; L$\infty$ Uniform Noise on RF; $\varepsilon \in \{0.001, 0.005, 0.01, 0.05, 0.1\}$, 10,000 stratified subsamples. Table~\ref{tab:adversarial} summarizes accuracy under adversarial attacks.
...
\label{fig:figure23}
\centering
\includegraphics[width=\columnwidth]{../results/E_LogicFlow.png}
\end{figure}
