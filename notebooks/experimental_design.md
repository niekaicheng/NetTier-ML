...
*防御结构的“不可能三角” (Impossible Trinity)*

![IDS 不可能三角](../results/E14_impossible_trinity_concept.png)

*注*: 传统的单体攻击检测模型往往在**高效率 (Efficiency)**、**高准确率 (Accuracy)** 和**高健壮性 (Robustness)** 这三个目标中难以兼得。例如，随机森林 (RF) 极速但对对抗噪声脆弱；深度学习 (DL) 准确但计算代价高且易受梯度攻击。分层架构通过利用模型的正交弱点，成功打破了这一三角约束，实现了三者的系统级平衡。
...
*注*: 雷达图综合评估了分层架构在**六个核心维度**上的多指标表现。结果显示，该系统成功打破了传统的“不可能三角”约束，实现了近乎理想的全方位帕累托最优权衡。
